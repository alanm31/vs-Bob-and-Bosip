...
class CustomControlsState extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		// bg
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image("option_bg"));
		bg.scrollFactor.x = 0;
		...
		leftArrow = new FlxSprite(inputvari.x - 60,inputvari.y - 10).loadGraphic(Paths.image("storymenu/arrowLeft"));

		rightArrow = new FlxSprite(inputvari.x + inputvari.width + 10, leftArrow.y).loadGraphic(Paths.image("storymenu/arrowRight"));
